...
public class ModelConstructor{	
	...
	private Inventory createInventory(Node invNode){
		...
		for(int i =0;i<items.getLength();i++){
			inventory.addItem((TakeableItem)createItem(items.item(i)),false);
		}
		return inventory;
	}
	private Occupation createOccupation(Node occNode){
		try{
			return Occupation.getOccupation(occNode.getTextContent());
		}catch(NullPointerException e){
			return null;
		}
	}
	...
	private LinkedList<Skill> createSkills(Node skillsNode){
		Element skillContainer = (Element)skillsNode;
		NodeList skilts = skillContainer.getElementsByTagName("skill");
		LinkedList<Skill> slist = new LinkedList<Skill>();
		for(int i =0;i<skilts.getLength();i++){
			slist.add(createSkill(skilts.item(i)));
		}
		return slist;
	}
	...
	private Equipment createEquipment(Node eqNode){
		...
		EquippableItem head = (EquippableItem)createItem(el.getElementsByTagName("head").item(0));
		EquippableItem armor = (EquippableItem)createItem(el.getElementsByTagName("armor").item(0));
		EquippableItem boot = (EquippableItem)createItem(el.getElementsByTagName("boot").item(0));
		EquippableItem weapon = (EquippableItem)createItem(el.getElementsByTagName("weapon").item(0));
		EquippableItem auxiliary = (EquippableItem)createItem(el.getElementsByTagName("auxiliary").item(0));
